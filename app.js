...
const IP_STORAGE = 'satriamail_v4_ip';
const DEFAULT_AVATAR = 'https://files.clugx.my.id/QO2mP.jpeg';

let currentIP = localStorage.getItem(IP_STORAGE);
if (!currentIP) {
    currentIP = 'ip_' + Math.random().toString(36).substring(2, 10);
    localStorage.setItem(IP_STORAGE, currentIP);
}
...
function renderAppShell() {
    ...
    root.innerHTML = `
        ...
        <div id="landingTab" class="tab-view active">
            <div class="hero-card">
...
function handleRoute() {
    ...
    const navMap = { '/home': 'home', '/inbox': 'inbox', '/sendv1': 'compose', '/public': 'public' };
...
function attachEvents() {
    ...
    document.getElementById('createAccountBtn').addEventListener('click', () => {
        ...
        const targetEmail = name.toLowerCase() + '@satria.dev';
        const isTaken = allUsers.some(u => u.email.toLowerCase() === targetEmail);
        
        if (isTaken) {
            alert('Nama sudah dipakai, silakan gunakan nama lain.');
            return;
        }
    ...
    document.getElementById('sendMsgBtn').addEventListener('click', () => {
        ...
        if (!rawTarget || !text) return alert('Isi semua field');
        
        const targetEmail = rawTarget.includes('@') ? rawTarget : rawTarget.toLowerCase() + '@satria.dev';
